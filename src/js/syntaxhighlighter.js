...
var SyntaxHighlighter = (function () {
      ...
      function decorate(sourceCode, position) {
        ...
        while (ti < nTokens) {
          ...
          if (isEmbedded) {
            ...
            appendDecorations(
              position + pos + embeddedSourceStart,
              embeddedSource,
              style,
              decorations
            );
            ...
            appendDecorations(
              position + pos + embeddedSourceEnd,
  ...
  SH.one = function (pre, language) {
    ...
    var sourceAndSpans = extractSourceSpans(pre),
      specificLanuage = (pre.className.match(/\bsh-(\w+)(?!\S)/i) || [
        0,
        null,
      ])[1];
    ...
    recombineTagsAndDecorations(
      sourceAndSpans,
      SH.findBrush(language)(sourceAndSpans.sourceCode, 0)
    );
  };
  ...
  SH.all = function (callback, parentNode) {
    ...
    for (i = 0; (node = nodes[i]); i++) {
      if (/\bsh\b/.test(node.className)) elements.push(node);
    }
    ...
    for (i = 0; (node = nodes[i]); i++) {
      if (/^code(\/|$)/.test(node.type)) {
        ...
        pre.className =
          'sh' + (language ? ' sh-' + (SH.langs[language] || language) : '');
  ...
  SH.highlight = function (sourceCode, language, startPosition) {
    var SH = SyntaxHighlighter,
      brush = SH.findBrush(language || SH.guessLanguage(sourceCode));
    if (brush) {
      ...
      for (; i < len; i += 2) {
        r +=
          ...
          decorations[i + 1] +
          '">' +
          sourceCode.substring(decorations[i], decorations[i + 2]) +
          '</span>';
      }
  ...
  var C_KEYWORDS =
    ...
    ' auto case char const default double enum extern float goto int long register short signed sizeof ' +
